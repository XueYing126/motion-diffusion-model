...
if __name__ == '__main__':
    ...
    parser.add_argument("--input_file", type=str, required=False, help='stick figure mp4 file to be rendered.')
    parser.add_argument("--input_folder", type=str, required=True, help='stick figure mp4 folder to be rendered.')
    parser.add_argument("--cuda", type=bool, default=True, help='')
    parser.add_argument("--device", type=int, default=0, help='')
    params = parser.parse_args()

    # for all files end with "_rep00.mp4", create the SMPL .npy
    folder_path = params.input_folder
    all_files = os.listdir(folder_path)
    mp4_files = [file for file in all_files if file.endswith('_rep00.mp4')]
    mp4_files.sort()

    for mp4_file in mp4_files:
        params.input_path = os.path.join(folder_path, mp4_file)
        assert params.input_path.endswith('.mp4')
        print(f"running {mp4_file} in {folder_path}")
