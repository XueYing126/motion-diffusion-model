    ...
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        ...
        for name in tqdm(id_list):
            try:
                ...
                # # motion is transformed to 263d during evaluation
                # motion = joints2263d(jtr.reshape(jtr.shape[0], 22, 3))
...
class TextOnlyDataset(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        text_data = random.choice(text_list)
        caption, tokens = text_data['caption'], text_data['tokens']
        return None, None, caption, None, np.array([0]), self.fixed_length, None, np.array([0])
...
class HumanML3D(data.Dataset):
    def __init__(self, mode, datapath='./dataset/humanml_opt.txt', split="train", **kwargs):
        ...
        if mode == 'gt':
            self.t2m_dataset = Text2MotionDatasetGT(self.opt, self.mean, self.std, self.split_file, self.w_vectorizer)
